...
const Detail = () => {
    ...
    useEffect(() => {
        const fetchEventData = async () => {
            try {
                const response = fetch(`https://app.ticketmaster.com/discovery/v2/events/${eventId}?apikey=${import.meta.env.VITE_TICKETMASTER_API_KEY}`)
    ...
    return(
        <div className={styles.container}>
            ...
            <div className={styles.seatInfoContainer}>
                ...
                <p className={styles.plaseNoteLegend} >{eventData.pleaseNote}</p>
                <p className={styles.rangePriceLegend} >Rango de precios: {eventData.priceRanges?.[0].min}-{eventData.priceRanges?.[0].max} {eventData.priceRanges?.[0].currency} </p>
            </div>
